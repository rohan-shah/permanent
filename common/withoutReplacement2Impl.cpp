...
namespace permanent
{
	...
	void withoutReplacement2Impl(withoutReplacementArgs& args)
	{
		...
		std::vector<mpfr_class> weights(1, 1.0/dimension), newWeights;
		...
		for(int permutationCounter = 0; permutationCounter < dimension; permutationCounter++)
		{
			...
			if((int)possibilities.size() <= n)
			{
				...
				newWeights.resize(possibilities.size());
				...
				for(int i = 0; i < (int)possibilities.size(); i++)
				{
					...
					if(newUsedRows[i*dimension + pos.newEntry])
					{
						...
						{
							newWeights[i] = weights[pos.parent] * alpha * std::fabs(matrix(pos.previousEntry, pos.newEntry)) / (dimension - permutationCounter - 1);
						}
						else
						{
							newWeights[i] = weights[pos.parent] * alpha * std::fabs(matrix(pos.previousEntry, pos.newEntry));
						}
					}
					else
					{
						newPreviousEntry[i] = pos.newEntry;
						newWeights[i] = weights[pos.parent] * std::fabs(matrix(pos.previousEntry, pos.newEntry));
					}
				}
			...
			else
			{
				...
				for(int i = 0; i < n; i++)
				{
					...
					if(newUsedRows[i*dimension + pos.newEntry])
					{
						...
						if(permutationCounter != dimension - 1)
						{
							newWeights[i] = weights[pos.parent] * alpha * std::fabs(matrix(pos.previousEntry, pos.newEntry))  / (samplingArgs.rescaledWeights[samplingArgs.indices[i]] * (dimension - permutationCounter - 1));
						}
						else
						{
					...
					if(previousEntry[i] == -1)
					{
						for(int j = 0; j < dimension - permutationCounter - 1; j++)
						{
							for(int k = 0; k < dimension - permutationCounter - 1; k++)
							{
								...
								{
									samplingArgs.weights.push_back(alpha * std::fabs(matrix(pos.previousEntry, pos.newEntry))/ (currentColumnSums[dimension*i + pos.newEntry] - std::fabs(matrix(pos.previousEntry, pos.newEntry))));
								}
								else
								{
									samplingArgs.weights.push_back(std::fabs(matrix(pos.previousEntry, pos.newEntry)) / (currentColumnSums[dimension*i + pos.newEntry] - std::fabs(matrix(pos.previousEntry, pos.newEntry))));
								}
							}
						}
